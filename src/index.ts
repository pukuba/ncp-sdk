import { NcloudSDKInput, SendMailInput, SendSMSInput, SendMailResponse, SendSMSResponse } from "./models"
...
export class NcloudSDK {
	...
	async sendSMS(args: SendSMSInput): Promise<SendSMSResponse> {
		...
		return await axios(`https://sens.apigw.ntruss.com/sms/v2/services/${this.smsKey}/messages`, {
			method: "POST",
	...
	private createHeader(service: string) {
		...
		const endPoint = service === "sms" ? `/sms/v2/services/${this.smsKey}/messages` : `/api/v1/mails`
